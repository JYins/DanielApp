...
### Resources And Newsletter Interaction QA: 2026-07-22

- Audited the four-tab iPhone 17 experience with current-run Simulator screenshots rather than code inspection alone. Accepted evidence is stored under `/private/tmp/daniel-ui-audit/` for Daily Verse, Connect, Resources, Settings, Newsletter card/full-screen/zoom states, and the PDF reader.
- Replaced the fixed-height, non-interactive Connect Newsletter image with an explicit full-screen media entry. The thumbnail keeps the warm card styling and adds a visible expand affordance and multi-image count.
- Added a trilingual full-screen Newsletter gallery with close control, page count, loading/failure states, a temporary gesture hint, double-tap zoom, native pinch zoom, zoomed-image panning, horizontal paging, and swipe-down dismissal.
- Resolved two issues found only through live interaction: the first UIKit image layout occurred at zero size and produced a black viewer, and the nested zoom scroll view initially captured one-finger paging. The final gesture arbitration gives one-finger horizontal movement to paging at 1x and to image panning when zoomed.
- Changed Hymn PDF/audio reading from an in-tab navigation destination to a full-screen cover. PDFKit continues to provide continuous pages, text accessibility, selection, scrolling, and pinch zoom while the audio player remains above the PDF when audio exists.
