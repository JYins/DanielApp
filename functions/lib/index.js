...
async function assertGlobalAdmin(context) {
    ...
    if (!isGlobalAdmin(caller.accessRole)) {
        throw new functions.https.HttpsError('permission-denied', 'Only global administrators can perform this action.');
    }
    return caller.uid;
}
async function loadAdminScope(context) {
    if (!context.auth) {
    ...
    const accessRole = normalizedAccessRole((callerData === null || callerData === void 0 ? void 0 : callerData.accessRole) || (callerData === null || callerData === void 0 ? void 0 : callerData.role), 'member');
    const membershipStatus = normalizedMembershipStatus(callerData === null || callerData === void 0 ? void 0 : callerData.membershipStatus, (callerData === null || callerData === void 0 ? void 0 : callerData.isApproved) === true ? 'active' : 'pending');
    if (!callerDoc.exists ||
        (callerData === null || callerData === void 0 ? void 0 : callerData.isApproved) !== true ||
        !['active', 'approved'].includes(membershipStatus) ||
        !['admin', 'global_admin', 'region_admin', 'branch_admin'].includes(accessRole)) {
        throw new functions.https.HttpsError('permission-denied', 'Only approved administrators can perform this action.');
    }
    return {
        uid: callerUid,
        accessRole,
        orgId: String((callerData === null || callerData === void 0 ? void 0 : callerData.orgId) || 'daniel-branch-church'),
        regionId: String((callerData === null || callerData === void 0 ? void 0 : callerData.regionId) || ''),
        branchId: String((callerData === null || callerData === void 0 ? void 0 : callerData.branchId) || ''),
    };
}
function normalizedAccessRole(value, fallback) {
    if (typeof value === 'string' && accessRoles.includes(value)) {
        return value;
    }
    return fallback;
}
function normalizedMembershipStatus(value, fallback) {
    if (typeof value === 'string' &&
        membershipStatuses.includes(value)) {
        return value;
    }
    return fallback;
}
function legacyRoleFor(accessRole) {
    return accessRole === 'global_admin' ? 'admin' : accessRole;
}
function isGlobalAdmin(accessRole) {
    return accessRole === 'admin' || accessRole === 'global_admin';
}
function targetCurrentRole(userData) {
    return normalizedAccessRole(userData.accessRole || userData.role, 'member');
}
function assertCanSetUserAccess(params) {
    const { caller, targetUid, targetData, branchId, branchData, accessRole, } = params;
    if (isGlobalAdmin(caller.accessRole)) {
        return;
    }
    if (targetUid === caller.uid) {
        throw new functions.https.HttpsError('invalid-argument', 'Scoped administrators cannot change their own access.');
    }
    const currentRole = targetCurrentRole(targetData);
    if (['admin', 'global_admin', 'region_admin'].includes(currentRole)) {
        throw new functions.https.HttpsError('permission-denied', 'Scoped administrators cannot manage global or regional administrators.');
    }
    if (!branchId || !branchData) {
        throw new functions.https.HttpsError('invalid-argument', 'Scoped administrator updates must include a valid branch.');
    }
    ...
    if (caller.accessRole === 'region_admin') {
        if (!caller.regionId || branchRegionId !== caller.regionId) {
            throw new functions.https.HttpsError('permission-denied', 'Region administrators can only manage users inside their region.');
        }
        if (currentRegionId && currentRegionId !== caller.regionId) {
            throw new functions.https.HttpsError('permission-denied', 'Region administrators cannot move users from another region.');
        }
        if (!['member', 'branch_admin'].includes(accessRole)) {
            throw new functions.https.HttpsError('permission-denied', 'Region administrators can only assign member or branch admin roles.');
        }
        return;
    }
    if (caller.accessRole === 'branch_admin') {
        if (!caller.branchId || branchId !== caller.branchId) {
            throw new functions.https.HttpsError('permission-denied', 'Branch administrators can only manage users inside their branch.');
        }
        if (currentBranchId && currentBranchId !== caller.branchId) {
            throw new functions.https.HttpsError('permission-denied', 'Branch administrators cannot move users from another branch.');
        }
        if (currentRole !== 'member' || accessRole !== 'member') {
            throw new functions.https.HttpsError('permission-denied', 'Branch administrators can only approve or revoke member access.');
        }
        return;
    }
    throw new functions.https.HttpsError('permission-denied', 'Your administrator role cannot perform this action.');
}
function membershipId(branchId, uid) {
...
exports.setUserAccessAdmin = functions
    .region('us-central1')
    .https.onCall(async (data, context) => {
    const caller = await loadAdminScope(context);
    ...
    if (!userDoc.exists) {
        throw new functions.https.HttpsError('not-found', `User profile ${uid} does not exist.`);
    }
    const userData = userDoc.data() || {};
    const currentAccessRole = normalizedAccessRole(userData.accessRole || userData.role, 'member');
    const accessRole = normalizedAccessRole(data.accessRole, currentAccessRole);
    const role = legacyRoleFor(accessRole);
    const isApproved = typeof data.isApproved === 'boolean' ? data.isApproved : userData.isApproved === true;
    const currentMembershipStatus = normalizedMembershipStatus(userData.membershipStatus, isApproved ? 'active' : 'pending');
    const membershipStatus = normalizedMembershipStatus(data.membershipStatus, isApproved ? 'active' : currentMembershipStatus);
    if (uid === caller.uid &&
        (!isApproved || !['admin', 'global_admin'].includes(accessRole))) {
        throw new functions.https.HttpsError('invalid-argument', 'You cannot revoke or demote your own global admin account.');
    }
    ...
    const orgId = String((branchData === null || branchData === void 0 ? void 0 : branchData.orgId) || userData.orgId || 'daniel-branch-church');
    const regionId = String((branchData === null || branchData === void 0 ? void 0 : branchData.regionId) || userData.regionId || '');
    const country = String((branchData === null || branchData === void 0 ? void 0 : branchData.country) || userData.churchCountry || regionName);
    const userUpdate = {
        isApproved,
    ...
    if (oldBranchId && oldBranchId !== branchId) {
        batch.set(db.collection('branchMemberships').doc(membershipId(oldBranchId, uid)), {
    ...
    if (branchId) {
        batch.set(db.collection('branchMemberships').doc(membershipId(branchId, uid)), {
